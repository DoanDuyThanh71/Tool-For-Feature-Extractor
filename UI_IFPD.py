...
import csv
import sys
from IFPD import appRun


class Worker(QtCore.QThread):
    finished = QtCore.pyqtSignal()

    def __init__(self, path, col, delta, row_selected):
        super().__init__()
        self.path = path
        self.col = col
    ...
    def run(self):
        shell = False
        appRun(self.path, self.col, self.valdelta, self.row_selected)
        self.finished.emit()
...
class UI_IFPD(object):
    ...
    def importData(self):
        ...
        if file_path:
            ...
            if os.path.exists(file_name):
                ...
                self.valdelta.setText(str(data_line[8]))
    ...
    def run_app(self):
        ...
        try:
            ...
            data = self.data
            self.row_selected = row_selected
            if row_selected <= 10 or row_selected >= row:
                error_dialog = ErrorDialog("")
            ...
            return
        self.worker = Worker(self.path, self.col, self.valdelta, self.row_selected)
    ...
    def setupUi(self, mainWindow):
        ...
        font.setFamily("Tahoma")
        font.setPointSize(12)
        self.ledSelectedRow.setFont(font)
        self.ledSelectedRow.setObjectName("ledSelectedRow")

        # Combo Box for selecting values
        self.valdelta = QtWidgets.QLabel(parent=self.centralwidget)
        self.valdelta.setGeometry(QtCore.QRect(160, 180, 131, 40))
        self.valdelta.setObjectName("delta")
        font = QtGui.QFont()
        font.setFamily("Tahoma")
        font.setPointSize(12)
        self.valdelta.setFont(font)
        self.valdelta.setObjectName("labelDelta")
    ...
    def retranslateUi(self, mainWindow):
        ...
        mainWindow.setWindowTitle(_translate("mainWindow", "Removing Object"))
